    ...
    it 'submits the unit to fleet' do
      expected_unit = {
        ...
        'Service' =>         { 'User' => 'root',
                               ...
                               'ExecStart' => '/opt/bin/systemd-docker run -p 3333 --rm --name awesome_0.0.1_e37496243_web-%i -e FOO=bar -e BAZ=qux.com -e PORT=3333 quay.io/assemblyline/awesome:0.0.1 bin/puma', # rubocop:disable Metrics/LineLength
                               'Restart' => 'always',
                               'RestartSec' => '10s',
                               'Type' => 'notify',
                               'NotifyAccess' => 'all',
      ...
      it 'submits each process' do
        ...
        worker_unit = {
          ...
          'Service' =>         { 'User' => 'root',
                                 'ExecStartPre' => ['/usr/bin/docker run --rm -v /opt/bin:/opt/bin ibuildthecloud/systemd-docker', # rubocop:disable Metrics/LineLength
                                                    '/usr/bin/docker pull quay.io/assemblyline/awesome:0.0.1'],
                                 'ExecStart' => '/opt/bin/systemd-docker run --rm --name awesome_0.0.1_e37496243_worker-%i -e FOO=bar -e BAZ=qux.com quay.io/assemblyline/awesome:0.0.1 bin/hutch', # rubocop:disable Metrics/LineLength
                                 'Restart' => 'always',
                                 'RestartSec' => '10s',
                                 'Type' => 'notify',
                                 'NotifyAccess' => 'all',
