  ...
  it 'can create its unit config' do
    expect(release.to_unit).to eq(
      ...
      'Service' =>         { 'User' => 'root',
                             ...
                             'ExecStart' => '/opt/bin/systemd-docker run -P --rm --name awesome-%i -e FOO=bar -e BAZ=qux.com quay.io/assemblyline/awesome:0.0.1', # rubocop:disable Metrics/LineLength
                             'Restart' => 'always',
                             'RestartSec' => '10s',
                             'Type' => 'notify',
                             'NotifyAccess' => 'all',
